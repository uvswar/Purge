...
function MFP:Awake(...)
  ...
  QBCore.Functions.TriggerCallback('MF_Purge:GetStartData', function(retVal) self.dS = true; self.cS = retVal; self:Start(); end)
end

function MFP:Start(...)
  -- while not ESX do Citizen.Wait(0); end
  if not self.dS or not self.cS then return; end
  QBCore.Functions.TriggerCallback('MF_Purge:GetPurgeState', function(isPurging,enemies,vehicles,loot)
...
function MFP:SpawnGroup(key)
  QBCore.Functions.TriggerCallback('MF_Purge:CanSpawn',function(can)
    if can then
...
function MFP:TrackGroup(spawnGroup,group)
  Citizen.CreateThread(function(...)
    while true do
      ...
      local players = QBCore.Functions.GetPlayersFromCoords(GetEntityCoords(leader), spawnGroup.EngageDist)
      local closest,dist
